...
void Timetable::on_pushButton_2_clicked()
{
    ...
    for(int i = 0; i < lst.count()/7; ++i){
        total +='\n';
        QString data = lst.at(i*7);
        int index = data.indexOf('.');
        while(index != -1){
            data[index] = '/';
            index = data.indexOf('.');
        }
        QString time = lst.at(i*7 + 2);
        QString timestart = time.split('-').at(0);
        QString timeend = time.split('-').at(1);
        timestart.insert(2,':');
        timeend.insert(2,':');
        QString all = lst.at(i*7 + 4) + "," + data + "," + timestart + "," + data + "," + timeend + "," + "False" + "," + lst.at(i*7 + 3) + "," + lst.at(i*7 + 5) + ",True";
        total += all;
    }
